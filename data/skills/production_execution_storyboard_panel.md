...
|------|------|--------|---------------------|----------------|
| **纯文本多参模式** | 仅写入视频描述与资产绑定，不生成提示词和分镜图 | `''`（空字符串） | `false` | 同「分镜图辅助多参模式」，同组累计时长必须是当前模型支持的合法时长 |
| **分镜图辅助多参模式** | 完整生成提示词并生成分镜图（当前默认行为） | 正常生成 | `true`（默认） | 同组累计时长必须是当前模型支持的合法时长 |
...
1. 获取 `script`、`storyboardTable` 和 `scriptPlan`，识别决策层指令中的**写入模式**（纯文本多参模式 / 分镜图辅助多参模式 / 首位帧模式），并读取 Assistant 上下文中的「视频模型时长硬约束」。每条 `duration` 必须与分镜表一致，且必须属于当前模型支持时长集合。**若为「分镜图辅助多参模式」或「首位帧模式」**：激活 `storyboard_prompt_techniques` 作为通用提示词技法参考（含解析映射规则、景别词库、输出格式规范、提示词结构框架、画质规范、图像资产标注规则、人物位置连贯性规则），并激活风格专属技法（`director_storyboard`）作为提示词生成的全部参考依据，冲突时以风格专属技法为准；**若为「纯文本多参模式」**：跳过提示词相关技法加载
2. 确定分组（track）与时长规则：
   - **纯文本多参模式 / 分镜图辅助多参模式**：同组内分镜 `duration` 累计值必须等于当前模型支持时长集合中的某个合法值；不得使用 15s 之类固定上限。若当前模型只支持 5s，则每组只能包含累计 5s 的分镜。
   - **首位帧模式**：**不分组**，每条分镜独立一组，`track` 按顺序递增（第1行 track=1，第2行 track=2，以此类推）；每条分镜 `duration` 仍必须是当前模型支持的合法值。
...
- 时长一致性约束：分镜面板 `duration` 必须与 `storyboardTable` 对应行时长完全一致，且必须属于当前视频模型支持时长集合。发现非法时长时，必须回到分镜表阶段拆分或修正，不得自行裁剪。
...
| `track` 分组 | 累计时长等于模型支持的合法值 | 累计时长等于模型支持的合法值 | 每行独立一组，按顺序递增 |
