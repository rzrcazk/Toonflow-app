...
1. 获取 `script`、`assets` 和 `scriptPlan`，并激活 `director_storyboard_table_narrative`、`director_storyboard_table_style` 作为分镜设计的风格参考，同时激活 `storyboard_table_techniques` 作为分镜表通用技法参考（含分镜拆分原则、定场与镜头合并规则、视觉连续性铁律、字段填写指引、转场规则）。
   - 同时读取 Assistant 上下文中的「视频模型时长硬约束」，建立当前模型支持时长集合，例如 `[5]`、`[5,10]`。后续每条分镜 `时长/duration` 必须取集合内的合法值。
...
- **导演规划对齐铁律**（每行可追溯，完成全表后整体自检）：
  ...
  - 高潮段（③节奏标记为"快"的段落）景别切换须更密、镜头平均时长更短；低密度段落保持定镜与中长时长，但任何分镜时长都必须落在当前模型支持时长集合内，禁止反向操作
  - `音效` 字段须与 ⑤声音方向 中该场标注的 1~2 个核心环境音一致，不得另立声源；规划标注的"沉默瞬间"对应行 `音效` 仅保留环境底噪或填 `静`
- **模型时长锁定**：分镜表 `时长` 列只能填写 Assistant 上下文「视频模型时长硬约束」列出的合法秒数。若当前模型只支持 `5` 秒，则所有分镜都必须为 `5` 秒；如果某个动作或台词不足 5 秒，也通过画面停顿、情绪反应或环境声补足为 5 秒，不输出 `1/2/3/4/6/8/10` 等非法值。
...
- **台词-时长强关联**：含台词的分镜，需根据角色当前情绪状态选取对应语速（愤怒~4字/秒、正常~3字/秒、悲伤~2字/秒、低语/虚弱~2字/秒），先估算 `台词字数 ÷ 语速（向上取整）+ 1s 情绪余量`，再选择当前模型支持集合内足以承载该内容的最小合法时长。若估算时长超过模型最大合法时长，须把台词拆分到多个连续分镜中，每个分镜时长都必须是合法值。
...
- **黄金 5 秒兼容**：当当前模型只支持 5s 时，所有分镜统一为 5s；当模型支持多个时长时，无台词镜头优先选择不超过 5s 的合法时长，定场/过渡类镜头尤其注意
