...
export default async (cells: { prompt: string }[], scriptId: number, projectId: number) => {
  const scriptData = await u.db("o_script").where({ id: scriptId, projectId }).first();
  const projectInfo = await u.db("o_project").where({ id: projectId }).first();

  const row = await u.db("o_outline").where({ id: scriptData?.outlineId!, projectId }).first();
  ...
  const imagesRaw = await u.db("o_assets").whereIn("name", resourceNames).andWhere({ projectId }).select("name", "type", "filePath");
