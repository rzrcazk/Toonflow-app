...
```
资产信息[id, type, name, describe, prompt, audio], ...
```
...
- **角色身份锁定**：多角色分镜必须用「姓名 + 外观锚点 + 服装/体型 + 固定站位 + 道具职责」识别角色。不要只写角色名。尤其同场出现两个以上同性/同服饰角色时，必须说明谁是宽脸壮实者、谁是高瘦者、谁持道具、谁堵门、谁只围观
- **禁止身份互换**：分镜正文必须加入一句身份约束，说明主要角色不可互换体型、站位、服装、道具和动作；群像/杂役不可替代主要角色
- **音色按三级优先处理**（有台词时必填）：① 角色资产音色描述（原文照搬不得润色）→ ② 角色资产参考音频（`@参考N` 绑定）→ ③ 都没有则按角色特征生成 9 维度描述
- **秒级时长控制**：单分镜时长最低 4s；videoDesc 低于 4s 时必须按 4s 输出或提示上游修正
- **即梦过审友好**：每个 4-5 秒分镜只保留一个主动作，提示词控制在 180-300 字左右；危险剧情降级为虚弱、惊醒、不适、梦魇、灵力紊乱、衣襟凌乱、尘痕或暗色旧污痕
- **动作降级**：不要写具体受伤部位、血液颜色/流动、伤口牵动，也不要写剧烈生理反应；把“强烈受伤表现”改成“略夸张地撑起身体、身体一僵、表情错愕、喘了口气、额角有汗”
- **明亮轻喜剧**：场景必须保持明亮暖色、画面清晰、旧屋但不阴森、轻喜剧反差；角色狼狈但不惨，紧张情节用错愕和尴尬反应呈现
- **安全收尾**：每条分镜正文末尾必须加入“画面健康明亮，无血腥、无惊悚、无危险行为、无新增角色、无文字水印。”
- **中文提示词**
...
参考定义:
@参考1: {资产1名字}，{从 describe/prompt 提取的可见身份锚点；角色写体型、脸型、服装、固定职责；场景/道具写可见形态}
@参考2: {资产2名字}，{从 describe/prompt 提取的可见身份锚点；角色写体型、脸型、服装、固定职责；场景/道具写可见形态}
@参考N: {资产N名字}，{简述}
...

角色身份锁定:
{角色A}: {外观锚点 + 服装 + 固定站位/职责 + 说话状态规则}
{角色B}: {外观锚点 + 服装 + 固定站位/职责 + 说话状态规则}
身份不可互换：{角色A} 与 {角色B} 不得交换体型、服装、位置、道具或动作；群像角色只作为背景围观，不替代主要角色。
...
画面健康明亮，无血腥、无惊悚、无危险行为、无新增角色、无文字水印。
```
...
分镜1 {N}s: 时间：{...}，场景：{场景名字}，镜头：{...}，{角色名字}只完成一个主动作，反应略夸张，有轻喜剧反差。画面健康明亮，无血腥、无惊悚、无危险行为、无新增角色、无文字水印。
分镜2 {N}s: 时间：{...}，场景：{场景名字}，镜头：{...}，{角色名字}只完成一个主动作，反应略夸张，有轻喜剧反差。画面健康明亮，无血腥、无惊悚、无危险行为、无新增角色、无文字水印。
...
```
...
7. **参考定义必须使用资产描述**：若资产信息中提供 `describe` 或 `prompt`，参考定义里的角色描述必须提取其中的可见身份锚点，不得只写「某某参考图」或只重复角色名
8. **角色身份锁定必须输出**：当分镜涉及两个及以上角色，或同一条提示词包含多个角色资产时，必须在「参考定义」后、「生成一个由以下...」前输出「角色身份锁定」段。该段必须明确每个主要角色的外观锚点、固定站位、道具职责、是否说话，并写明身份不可互换
9. **分镜正文禁用 `@参考N `**：正文统一使用角色名/场景名，不写 `@参考1/@参考2` 等编号；但每次提到角色动作时，必须尽量携带一个短身份锚点，例如「宽脸壮实的王师兄」「高瘦手持生死状的周师兄」
10. **单分镜时长最低 4s**
11. **时长单位**：直接使用 videoDesc 中的秒数，格式为 `{N}s`（如 `4s`），最低 4s；若 videoDesc 低于 4s，按 4s 输出，不得生成 1~3s 分镜段落
12. **参考类型严格区分**：音色绑定（`音色：@参考N`）只能指向音频参考；参考定义里图片条目独占一行（`@参考K: 名字，描述`），若该资产带 audio，则在同一行尾部追加「，参考音频为：@参考K+1」标记其归属，音频本身不另起行但仍占用编号 K+1；禁止把音频参考写在图片角色描述位上，禁止把图片参考用作音色绑定
...
```
```xml
<storyboardItem videoDesc='（沈辞独立城楼远眺开阔山色、城楼、沈辞/城楼、4s、全景、静止、负手而立衣袂随风轻动、错愕但镇定、明亮暖色柔和天光、无台词、轻微环境声和衣料声、A001/A003）' prompt='全景，平视略仰，城楼之上，沈辞负手而立，衣袂轻动，明亮暖色柔和天光...' track='main' duration='4' associateAssetsIds="[&quot;A001&quot;,&quot;A003&quot;]" shouldGenerateImage="true" ></storyboardItem>
...
输出：
```
画面风格和类型: 国风3D轻喜剧玄幻短剧质感, 明亮暖色, 画面清晰, 旧屋但不阴森
...
分镜1 4s: 时间：清晨，场景：城楼，镜头：全景，平视略仰，静止镜头，沈辞独立城楼之上，负手而立，衣袂随风轻动，目光远眺开阔山色，神情错愕但镇定。无台词。背景是古城楼砖石纹理清晰，远方山色明朗，明亮暖色柔和天光，画面清晰，有轻喜剧反差。镜头静止。画面健康明亮，无血腥、无惊悚、无危险行为、无新增角色、无文字水印。
