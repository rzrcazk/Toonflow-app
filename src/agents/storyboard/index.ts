...
export default class Storyboard {
  ...
  getScript = tool({
    ...
    execute: async () => {
      ...
      const script = await u.db("o_script").where({ id: this.scriptId, projectId: this.projectId }).first();
  ...
  getAssets = tool({
    ...
    execute: async () => {
      ...
      const scriptData = await u.db("o_script").where({ id: this.scriptId, projectId: this.projectId }).first();
      const row = await u.db("o_outline").where({ id: scriptData?.outlineId!, projectId: this.projectId }).first();
...
3. 禁止捏造资产列表中不存在的角色、场景、道具`;
  ...
  private async buildEnvironmentContext(): Promise<string> {
    const projectInfo = await u.db("o_project").where({ id: this.projectId }).first();

    const row = await u.db("o_outline").where({ id: this.scriptId, projectId: this.projectId }).first();
    const outline: any | null = row?.data ? JSON.parse(row.data) : null;
...
${task}
  ...
  private async invokeSubAgent(agentType: AgentType, task: string): Promise<string> {
    ...
    const promptsList = await u.db("o_prompts").where("code", "in", ["storyboard-segment", "storyboard-shot"]);
  ...
  async call(msg: string): Promise<string> {
    ...
    const prompts = await u.db("o_prompts").where("code", "storyboard-main").first();
    const promptConfig = await u.getPromptAi("storyboardAgent");
