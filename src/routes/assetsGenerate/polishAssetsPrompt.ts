...
export default router.post(
  ...
  async (req, res) => {
    ...
    const promptsList = await u.db("o_prompt").where("code", "in", ["role-polish", "scene-polish", "storyboard-polish", "tool-polish"]);
    ...
    if (type == "role") {
      ...
      const novelData = (await u.db("o_novel").whereIn("chapterIndex", chapterRange).select("*")) as NovelChapter[];
