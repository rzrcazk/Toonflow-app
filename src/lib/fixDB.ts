...
import { DEFAULT_IMAGE_MODEL, isLegacyJimengImageModel } from "@/utils/defaultImageModel";
import { DEFAULT_VIDEO_MODE, DEFAULT_VIDEO_MODEL, isLegacyJimengSeedanceVipModel } from "@/utils/defaultVideoModel";
import { SCRIPT_ASSET_EXTRACTION_PROMPT } from "@/utils/scriptAssetExtractionPrompt";
...
export default async (knex: Knex): Promise<void> => {
  ...
  const openai = u.vendor.getVendor("openai");
  const openaiCode = u.vendor.getCode("openai");
  const isOpenAIDefaultTemplateNeedingUpgrade =
    Number(openai?.version || 0) < 2.2 &&
    openai?.models?.some((model: any) => model.modelName === "gpt-5.5") &&
    !openai?.models?.some((model: any) => model.modelName === "gpt-image-2" && model.mode?.includes("multiReference")) &&
    /name:\s*"GPT-5\.5"[\s\S]*modelName:\s*"gpt-5\.5"/.test(openaiCode) &&
    (/const imageRequest[\s\S]*return\s+"";[\s\S]*const videoRequest/.test(openaiCode) ||
      /modelName:\s*"gpt-image-2"[\s\S]*mode:\s*\["text",\s*"singleImage"\]/.test(openaiCode));
  if (isOpenAIDefaultTemplateNeedingUpgrade) {
    u.vendor.writeCode("openai", vendorData["openai.ts"]);
  }
  const jimeng = u.vendor.getVendor("jimeng");
  if (Number(jimeng?.version || 0) < 1.1) {
    u.vendor.writeCode("jimeng", vendorData["jimeng.ts"]);
  }

  if ((await knex.schema.hasTable("o_project")) && (await knex.schema.hasColumn("o_project", "imageModel"))) {
    const projects = await db("o_project").select("id", "imageModel");
    const projectIds = projects
      .filter((project: any) => !String(project.imageModel ?? "").trim() || isLegacyJimengImageModel(project.imageModel))
      .map((project: any) => project.id);
    if (projectIds.length) {
      await db("o_project").whereIn("id", projectIds).update({ imageModel: DEFAULT_IMAGE_MODEL });
    }
  }
  if (
    (await knex.schema.hasTable("o_project")) &&
    (await knex.schema.hasColumn("o_project", "videoModel")) &&
    (await knex.schema.hasColumn("o_project", "mode"))
  ) {
    const projects = await db("o_project").select("id", "videoModel", "mode");
    const projectIds = projects
      .filter((project: any) => isLegacyJimengSeedanceVipModel(project.videoModel) && String(project.mode ?? "").trim() === "startFrameOptional")
      .map((project: any) => project.id);
    if (projectIds.length) {
      await db("o_project").whereIn("id", projectIds).update({
