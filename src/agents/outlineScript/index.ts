...
export default class OutlineScript {
  ...
  private async getProjectInfo(): Promise<any> {
    return u.db("o_project").where({ id: this.projectId }).first();
  }
  ...
  private async findStoryline() {
    return u.db("o_storyline").where({ projectId: this.projectId }).first();
  }

  private async upsertStorylineContent(content: string) {
    const existing = await this.findStoryline();
    if (existing) {
      await u.db("o_storyline").where({ projectId: this.projectId }).update({ content });
    } else {
      await u.db("o_storyline").insert({ projectId: this.projectId, content });
    }
    this.refresh("storyline");
  }

  private async deleteStorylineContent() {
    const deleted = await u.db("o_storyline").where({ projectId: this.projectId }).del();
    this.refresh("storyline");
    return deleted;
  }
  ...
  private async findOutlines() {
    return u.db("o_outline").where({ projectId: this.projectId }).orderBy("episode", "asc");
  }

  private async findOutlineById(id: number) {
    return u.db("o_outline").where({ id, projectId: this.projectId }).first();
  }

  private async getMaxEpisode(): Promise<number> {
    const result: any = await u.db("o_outline").where({ projectId: this.projectId }).max("episode as max").first();
    return result?.max ?? 0;
  }

  private async clearOutlinesAndScripts() {
    const outlines = await u.db("o_outline").select("id").where({ projectId: this.projectId });
    if (outlines.length === 0) return 0;

    const outlineIds = outlines.map((o) => o.id);
    await u.db("o_script").whereIn("o_script.outlineId", outlineIds).del();
    await u.db("o_outline").where({ projectId: this.projectId }).del();
  ...
  private async insertOutlines(episodes: EpisodeData[], startEpisode: number) {
    ...
    await u.db("o_outline").insert(insertList);
  ...
  private async createEmptyScripts(outlineIds: Array<{ id: number; data: string }>) {
    ...
    if (scripts.length > 0) {
      await u.db("o_script").insert(scripts);
    }
...
道具(按出场顺序): ${ep.props?.map((p: AssetItem) => `${p.name}(${p.description})`).join("; ") || "无"}`;
  ...
  private async findAssetByTypeAndName(type: AssetType, name: string) {
    return u.db("o_assets").where({ projectId: this.projectId, type, name }).first();
  }
  ...
  private async upsertAsset(type: AssetType, item: AssetItem): Promise<"inserted" | "updated" | "skipped"> {
    ...
    if (existing.describe !== item.description) {
  ...
  private async generateAssetsFromOutlines() {
    const outlines = await u.db("o_outline").select("data").where({ projectId: this.projectId });
  ...
  private async buildEnvironmentContext(): Promise<string> {
    const [novelInfo, storyline, outlineCount] = await Promise.all([
      ...
      this.findStoryline(),
      u.db("o_outline").where({ projectId: this.projectId }).count("id as count").first() as any,
    ]);
...
${task}
  ...
  private async invokeSubAgent(agentType: AgentType, task: string): Promise<string> {
    ...
    const promptsList = await u.db("o_prompts").where("code", "in", ["outlineScript-a1", "outlineScript-a2", "outlineScript-director"]);
  ...
  async call(msg: string): Promise<string> {
    ...
    const prompts = await u.db("o_prompts").where("code", "outlineScript-main").first();
    const promptConfig = await u.getPromptAi("outlineScriptAgent");
